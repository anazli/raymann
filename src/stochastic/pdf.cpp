...
float CosPdf::value(const Vec3f &direction) const {
  auto cosTheta = dot(normalized(direction), onb.w());
...
float PrimitivePdf::value(const Vec3f &direction) const { return 1.f; }

Vec3f PrimitivePdf::generate() const { return Vec3f(1.f, 1.f, 1.f); }
