...
void ShapeBuilder::addTransformer() {
  m_currentTraceable = new Transformer(m_currentTraceable, m_prop.objTrans());
}
...
void ShapeBuilder::addStripePattern() {
  m_currentTraceable =
      new StripePattern(m_currentTraceable, m_prop.patColA(), m_prop.patColB(),
                        m_prop.objTrans(), m_prop.patTrans());
}

void ShapeBuilder::addGradientPattern() {
  m_currentTraceable = new GradientPattern(m_currentTraceable, m_prop.patColA(),
                                           m_prop.patColB(), m_prop.objTrans(),
                                           m_prop.patTrans());
}

void ShapeBuilder::addRingPattern() {
  m_currentTraceable =
      new RingPattern(m_currentTraceable, m_prop.patColA(), m_prop.patColB(),
                      m_prop.objTrans(), m_prop.patTrans());
}
