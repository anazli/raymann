...
TEST_F(ApplicationTest, parseValidInput) {
  ...
  compareVectors(reader.vertexCollection()[0], Vec3D(1.f, 2.f, 3.f));
  compareVectors(reader.vertexCollection()[1], Vec3D(-1.f, -2.f, -3.f));
  compareVectors(reader.vertexCollection()[2], Vec3D(1.5f, 4.5f, -0.3f));
  compareVectors(reader.vertexCollection()[3], Vec3D(-4.05f, 1.053f, 0.0005f));

  ASSERT_TRUE(reader.triangleCollection().size() == 2);
  comparePoints(reader.triangleCollection()[0].point(0),
                Point3D(reader.vertexCollection()[0]));
  comparePoints(reader.triangleCollection()[0].point(1),
                Point3D(reader.vertexCollection()[1]));
  comparePoints(reader.triangleCollection()[0].point(2),
                Point3D(reader.vertexCollection()[2]));
  ...
  comparePoints(reader.triangleCollection()[1].point(2),
                Point3D(reader.vertexCollection()[3]));
}
...
TEST_F(ApplicationTest, parseValidInputWithVertexNormals) {
  ...
  compareVectors(reader.vertexNormalCollection()[0], Vec3D(0.f, 0.f, 1.f));
  compareVectors(reader.vertexNormalCollection()[1],
                 Vec3D(0.707f, 0.f, -0.707f));
  compareVectors(reader.vertexNormalCollection()[2], Vec3D(1.f, 2.f, 3.f));
}
...
TEST_F(ApplicationTest, parseValidPolygonInput) {
  ...
  comparePoints(reader.triangleCollection()[2].point(2),
                Point3D(reader.vertexCollection()[4]));
}
...
TEST_F(ApplicationTest, parseFileWithGroups) {
  ...
  if (it.first()) {
    ...
    if (it1.first()) {
      ...
      if (t1) {
        comparePoints(t1->point(0), Point3D(reader.vertexCollection()[0]));
        comparePoints(t1->point(1), Point3D(reader.vertexCollection()[1]));
        comparePoints(t1->point(2), Point3D(reader.vertexCollection()[2]));
      }
    }
    ...
    if (it2.first()) {
      ...
      if (t2) {
        comparePoints(t2->point(0), Point3D(reader.vertexCollection()[0]));
        comparePoints(t2->point(1), Point3D(reader.vertexCollection()[2]));
        comparePoints(t2->point(2), Point3D(reader.vertexCollection()[3]));
      }
    }
  }
}
...
TEST_F(ApplicationTest, parseFileWithSmoothTriangles) {
  ...
  if (it.first()) {
    ...
    comparePoints(t1->points(0), Point3D(reader.vertexCollection()[0]));
    comparePoints(t1->points(1), Point3D(reader.vertexCollection()[1]));
    comparePoints(t1->points(2), Point3D(reader.vertexCollection()[2]));
    ...
    comparePoints(t2->points(0), Point3D(reader.vertexCollection()[0]));
    comparePoints(t2->points(1), Point3D(reader.vertexCollection()[1]));
    comparePoints(t2->points(2), Point3D(reader.vertexCollection()[2]));
