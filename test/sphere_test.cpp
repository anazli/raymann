...
TEST_F(Tsphere, raySphereIntersection) {
  ...
  ASSERT_EQ(s.getRecord().count, 2);
  ASSERT_EQ(s.getRecord().t1, 4.0f);
  ASSERT_EQ(s.getRecord().t2, 6.0f);
}
...
TEST_F(Tsphere, raySphereTangentIntersection) {
  ...
  ASSERT_TRUE(s.intersect(r));
  ASSERT_EQ(s.getRecord().count, 2);
  ASSERT_EQ(s.getRecord().t1, 5.0f);
  ASSERT_EQ(s.getRecord().t2, 5.0f);
}
...
TEST_F(Tsphere, raySphereNoIntersection) {
  ...
  ASSERT_EQ(s.getRecord().count, 0);
  ASSERT_EQ(s.getRecord().t1, -1.0f);
  ASSERT_EQ(s.getRecord().t2, -1.0f);
}
...
TEST_F(Tsphere, SphereIsBehindOrigin) {
  ...
  ASSERT_TRUE(s.intersect(r));
  ASSERT_EQ(s.getRecord().count, 2);
  ASSERT_EQ(s.getRecord().t1, -6.0f);
  ASSERT_EQ(s.getRecord().t2, -4.0f);
}
...
TEST_F(Tsphere, testingTheNextTest) {
  ...
  ASSERT_EQ(s.getRecord().count, 2);
  ASSERT_EQ(s.getRecord().t1, 3.0f);
  ASSERT_EQ(s.getRecord().t2, 7.0f);
}
...
TEST_F(Tsphere, transformsSphere) {
  ...
  ASSERT_EQ(t->getRecord().count, 2);
  ASSERT_EQ(t->getRecord().t1, 3.0f);
  ASSERT_EQ(t->getRecord().t2, 7.0f);
  delete t;
}
...
TEST_F(Tsphere, translatesSphere) {
  ...
  ASSERT_TRUE(t->getRecord().count == 0);
  delete t;
}
